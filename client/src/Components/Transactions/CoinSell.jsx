...
export default function CoinSell() {
  ...
  const getusertransaction_byQuantity = async () => {
    ...
    try {
      console.log("Processing sell transaction by quantity...");
      await getamount();
      ...
      const response = await axios.post("http://localhost:3004/wallet/sellTransaction", {
        Quantity: Quantity,
        Amount: (`${state.data.current_price}` / 100) * 70 * Quantity,
        login: token,
        CoinName: data.name,
  ...
  const getusertransaction_byAmount = async () => {
    ...
    try {
      console.log("Processing sell transaction by amount...");
      await getamount();
